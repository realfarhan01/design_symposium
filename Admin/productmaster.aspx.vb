...
Partial Class Admin_productmaster
    ...
    Protected Sub btnsubmit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSubmit.Click
        Try
            ...
            Dim Result As String = BLL.AddUpdate_Product(hfId.Value, user, File, ddlCategory.SelectedValue, FolderPath, txtProductName.Text, "", "", ddlStatus.SelectedValue)
            showdata()
            div1.Visible = False
            div2.Visible = True
            hfId.Value = 0
    ...
    Protected Sub DataDisplay_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles DataDisplay.RowCommand
        If e.CommandName = "Delete1" Then
            BLL.ExecNonQuery("Update tbl_Product Set Deactivated=2 Where cnt=@cnt and UserName=@UserName", "@cnt", e.CommandArgument, "@UserName", System.Configuration.ConfigurationManager.AppSettings("UserName"))
            showdata()
        ElseIf e.CommandName = "Deactivate1" Then
            hfId.Value = e.CommandArgument
            Dim dr As SqlDataReader = BLL.ExecDataReader("select * from tbl_Product where cnt=@cnt and UserName=@UserName", "@cnt", e.CommandArgument, "@UserName", System.Configuration.ConfigurationManager.AppSettings("UserName"))
            If dr.Read Then
                div1.Visible = True
                div2.Visible = False
    ...
    End Sub
    Sub showdata()
        DataDisplay.DataSource = BLL.ExecDataTableProc("Prc_GetProducts", "@UserName", System.Configuration.ConfigurationManager.AppSettings("UserName"))
